...
public class SugarCaneFeature {
    ...
    public static BPos findSugarCaneStack(long populationSeed, int chunkX, int chunkZ, int minHeight, int startRootY, BPos dirtPos, ChunkRand rand) {
        int dirtX = dirtPos.getX();
        int dirtZ = dirtPos.getZ();
        ...
        if (overlapBox.minX <= dirtX && dirtX <= overlapBox.maxX) {
            if (overlapBox.minZ <= dirtZ && dirtZ <= overlapBox.maxZ) {
                sugarCaneStacks.add(new BPos(dirtX, startRootY, dirtZ));
            }
        }
